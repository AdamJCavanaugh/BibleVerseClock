...
class Bible {
    async loadBible() {
        try {
            ...
            const verses = this.parseBibleText(bibleText);
            return verses;
        } catch (error) {
    ...
    getVerseByTime(verses) {
        ...
        if (cleanMinutes === '0') {
            ...
            matchingVerse = this.getRandomVerse(verses);
        } else {
...
class App {
    async init() {
        ...
        const verses = await bible.loadBible();
        bible.getVerseByTime(verses);
        setInterval(() => bible.getVerseByTime(verses), 60000); // Update verse every minute
    }
}
